...
setwd("C:/Users/tamec/Desktop/Programming/Git/ExpMeth Assignment 3 Data")

library(pacman)
p_load(Metrics, caret, groupdata2, pastecs, data.table, perturb, lmerTest, ggplot2, plyr, dplyr, tidyverse, MuMIN, stringr, cvTools, hydroGOF)


ass1data=read.csv("df.csv")
demotest=read.csv("demo_test.csv")
LUtest=read.csv("LU_test.csv")
tokentest=read.csv("token_test.csv")

names(demotest)[names(demotest)=="Child.ID"] <-"ID"
names(LUtest)[names(LUtest)=="SUBJ"] <-"ID"
names(tokentest)[names(tokentest)=="SUBJ"] <-"ID"
...
mean$Child.ID = c("Average TD","Average TD","Average TD","Average TD","Average TD","Average TD")

bernie=merge(mean,bernie, all = T)

bernie$Child.ID=as.factor(bernie$Child.ID)
...
td = typically_children %>%
    group_by(Visit) %>%
    dplyr::summarize(CHI_MLU = mean(CHI_MLU), verbalIQ = mean(verbalIQ),MOT_MLU = mean(MOT_MLU),nonVerbalIQ = mean(nonVerbalIQ),Ethnicity = "White",Diagnosis="TD",Child.ID="predicted TD" )
...
td$CHI_MLU = predict(vogue,td,allow.new.levels=T)

everything = merge(td,bernie,all = T)


ggplot(everything,aes(Visit,CHI_MLU,fill = Child.ID))+geom_bar(stat="identity",position = "dodge")

abs(bernie$CHI_MLU[bernie$Child.ID=="Average TD"] - bernie$CHI_MLU[bernie$Child.ID=="Bernie"])

everything$CHI_MLU[everything$Child.ID=="Average TD"]

everything$CHI_MLU[everything$Child.ID =="predicted TD"[everything$VISIT =="6"]]
```
